...
annual_vars[ANNUAL_VARS_COUNT] =
{
	...
	, { "N_SEED", "Numberseed", "Number of seeds", "seeds ha-1"} // Added by Saponaro
   ...
   const int deflate_level = 9;
	...
	for ( i = 0; i < dataset->columns_count; ++i )
	{
		...
		for ( year = 0; year < years_loop; ++year )
		{
			...
			j = sprintf(buf2,"%s",(ANNUAL_DATASET_TYPE == dataset->type) ? annual_vars[i].standard_name : daily_vars[i].long_name);
			if ( (ret = nc_put_att_text(nc_id, var_id, sz_long_name, strlen(buf2), buf2)) )
				goto err;

			sprintf(buf2,"%s",(ANNUAL_DATASET_TYPE == dataset->type) ? annual_vars[i].unit : daily_vars[i].unit);
