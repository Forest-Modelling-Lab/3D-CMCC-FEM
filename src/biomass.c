	...
	for ( height = 0; height < c->heights_count ; ++height )
	{
		...
		for ( dbh = 0; dbh < c->heights[height].dbhs_count; ++dbh )
		{
			for ( age = 0; age < c->heights[height].dbhs[dbh].ages_count ; ++age )
			{
				...
				//	s->value[TREE_SEEDS_C]               = (s->value[SEEDS_C]            / (double)s->counter[N_TREE]);
				//  s->value[TREE_SEEDLINGS_C]           = (s->value[SEEDLINGS_C]        / (double)s->counter[N_TREE]);
				//  s->value[TREE_SAPLINGS_C]            = (s->value[SAPLINGS_C]         / (double)s->counter[N_TREE]);
...
void tree_biomass_remove (cell_t *const c, const int height, const int dbh, const int age, const int species, const int tree_remove, const int nat_man)
{
	...
	//s->value[C_SEEDS_TO_CWD]     += (s->value[TREE_SEEDS_C] * tree_remove);

	//s->value[C_SEEDLINGS_TO_CWD]     += (s->value[TREE_SEEDLINGS_C] * tree_remove);

	//s->value[C_SAPLINGS_TO_CWD]     += (s->value[TREE_SAPLINGS_C] * tree_remove);
