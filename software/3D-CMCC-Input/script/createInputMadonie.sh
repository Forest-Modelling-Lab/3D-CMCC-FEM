...
#OUTPUT_02="${WORK_DIR}/Madonie.tif"
#log "${MSG} ...\n"
...
#OUTPUT_03="${WORK_DIR}/Madonie_remap.tif"
#log "${MSG} ...\n"
...
#for INPUT_02 in $( ls ${IN_00}/*.asc ) ; do
	...
	#	OUTPUT_04="${WORK_DIR}/$( basename $( echo ${INPUT_02} | sed s/.asc/.tif/ ) )"
	#	log "${MSG} ...\n"
	...
	#	OUTPUT_05="${WORK_DIR}/$( basename $( echo ${INPUT_02} | sed s/.asc/_zone33.tif/ ) )"
	#	log "${MSG} ...\n"
	#	gdalwarp ${PAR_01} -t_srs "${PROJ}" ${OUTPUT_04} ${OUTPUT_05} &>> "${LOGFILE}"
	#	check "${MSG} failed on ${OUTPUT_04}.\n"

	#	MSG="Remap of UTM geotiff image"
	#	OUTPUT_06="${WORK_DIR}/$( basename $( echo ${INPUT_02} | sed s/.asc/_Madonie_30m.tif/ ) )"
	#	log "${MSG} ...\n"	
