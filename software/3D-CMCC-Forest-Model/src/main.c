...
static int log_start(const char* const sitename) {
	...
	/* create output files */
	g_log = logger_new_ex(debug_output_file, buffer);
	if ( ! g_log ) puts("Unable to log to file: check logfile path!");
	
	g_daily_log = logger_new_ex(daily_output_file, buffer);
	if ( ! g_daily_log ) puts("Unable to create daily log!");
	
	g_monthly_log = logger_new_ex(monthly_output_file, buffer);
	if ( ! g_monthly_log ) puts("Unable to create monthly log!");

	g_annual_log = logger_new_ex(annual_output_file, buffer);
	if ( ! g_annual_log ) puts("Unable to create annual log!");
	
	g_soil_log = logger_new_ex(soil_output_file, buffer);
	...
	if ( sitename && sitename[0] ) {
		strcat (debug_output_file, "_");
		strcat (debug_output_file, sitename);

		strcat (daily_output_file, "_");
		strcat (daily_output_file, sitename);

		strcat (monthly_output_file, "_");
		strcat (monthly_output_file, sitename);

		strcat (annual_output_file, "_");
		strcat (annual_output_file, sitename);

		strcat (soil_output_file, "_");
		strcat (soil_output_file, sitename);
	}
	...
	strcat (debug_output_file, strTmp);
	strcat (debug_output_file, strTmp2);
	strcat (debug_output_file, strTmp3);

	strcat (daily_output_file, strTmp);
	strcat (daily_output_file, strTmp2);
	strcat (daily_output_file, strTmp3);

	strcat (monthly_output_file, strTmp);
	strcat (monthly_output_file, strTmp2);
	strcat (monthly_output_file, strTmp3);

	strcat (annual_output_file, strTmp);
	strcat (annual_output_file, strTmp2);
	strcat (annual_output_file, strTmp3);

	strcat (soil_output_file, strTmp);
	strcat (soil_output_file, strTmp2);
	strcat (soil_output_file, strTmp3);


	sprintf(strSizeCell, "%d", (int)settings->sizeCell);

	strcat (debug_output_file, strSizeCell);
	strcat (debug_output_file, "_");

	strcat (daily_output_file, strSizeCell);
	strcat (daily_output_file, "_");

	strcat (monthly_output_file, strSizeCell);
	strcat (monthly_output_file, "_");

	strcat (annual_output_file, strSizeCell);
	strcat (annual_output_file, "_");

	strcat (soil_output_file, strSizeCell);
	strcat (soil_output_file, "_");
	...
	strcat (debug_output_file, strData);
	strcat (debug_output_file, "_");

	strcat (daily_output_file, strData);
	strcat (daily_output_file, "_");

	strcat (monthly_output_file, strData);
	strcat (monthly_output_file, "_");

	strcat (annual_output_file, strData);
	strcat (annual_output_file, "_");

	strcat (soil_output_file, strData);
	strcat (soil_output_file, "_");
	...
	/* add suffix for type files */
	{
		...
		strcat(debug_output_file, ext);
		strcat(daily_output_file, ext);
		strcat(monthly_output_file, ext);
		strcat(annual_output_file, ext);
		strcat(soil_output_file, ext);
	}

	strcat (debug_output_file, ".txt");
	strcat (daily_output_file, ".txt");
	strcat (monthly_output_file, ".txt");
	strcat (annual_output_file, ".txt");
	strcat (soil_output_file, ".txt");

	/* create output files */
	g_log = logger_new(debug_output_file);
	if ( ! g_log ) puts("Unable to log to file: check logfile path!");
	
	g_daily_log = logger_new(daily_output_file);
	if ( ! g_daily_log ) puts("Unable to create daily log!");
	
	g_monthly_log = logger_new(monthly_output_file);
	if ( ! g_monthly_log ) puts("Unable to create monthly log!");

	g_annual_log = logger_new(annual_output_file);
	if ( ! g_annual_log ) puts("Unable to create annual log!");
	
	g_soil_log = logger_new(soil_output_file);
	if ( ! g_soil_log ) puts("Unable to create soil log!");
#endif
	...
	printf(msg_debug_output_file, debug_output_file);
	printf(msg_daily_output_file, daily_output_file);
	printf(msg_monthly_output_file, monthly_output_file);
...
static int parse_args(int argc, char *argv[]) {
	...
	monthly_output_file = NULL;
	annual_output_file = NULL;
	soil_output_file = NULL;
	dataset_file = NULL;
	input_met_path = NULL;
	soil_path = NULL;
	topo_path = NULL;
	...
	for ( i = 1; i < argc; ++i ) {
		...
		switch ( argv[i][1] ) {
			...
			case 'o': /* debug file path */
				debug_output_file = string_copy(argv[i+1]);
				if ( ! debug_output_file ) {
					fprintf(stderr, "cannot allocate memory for debug_output_file.\n");
					goto err;
				}
			break;

			case 'b': /* daily file path */
				daily_output_file = string_copy(argv[i+1]);
				if( ! daily_output_file ) {
					fprintf(stderr, "cannot allocate memory for daily_output_file.\n");
					goto err;
				}
			break;

			case 'f': /* monthly file path */
				monthly_output_file = string_copy(argv[i+1]);
				if( ! monthly_output_file ) {
					fprintf(stderr, "cannot allocate memory for monthly_output_file.\n");
					goto err;
				}
			break;

			case 'e': /* yearly file path */
				annual_output_file = string_copy(argv[i+1]);
				if( ! annual_output_file ) {
					fprintf(stderr, "cannot allocate memory for annual_output_file.\n");
					goto err;
				}
			break;

			case 'n': /* soil file path */
				soil_output_file = string_copy(argv[i+1]);
				if( ! soil_output_file ) {
					fprintf(stderr, "cannot allocate memory for soil_output_file.\n");
					goto err;
				}
			break;
			...
			case 'c': /* settings file */
				settings_path = string_copy(argv[i+1]);
				if( ! settings_path ) {
...
int main(int argc, char *argv[])
{
	...
	if ( output_vars && output_vars->daily_vars_value ) {
		int ret;
		char *path = get_path(daily_output_file);
		if ( ! path && daily_output_file  ) {
			logger(g_log, err_out_of_memory);
			matrix_free(m);
			return 1;
		}
	...
	if ( output_vars && output_vars->monthly_vars_value ) {
		int ret;
		char *path = get_path(monthly_output_file);
		if ( ! path && monthly_output_file ) {
			logger(g_log, err_out_of_memory);
			matrix_free(m);
			return 1;
		}
	...
	if ( output_vars && output_vars->yearly_vars_value ) {
		int ret;
		char *path = get_path(annual_output_file);
		if ( ! path && annual_output_file) {
			logger(g_log, err_out_of_memory);
			matrix_free(m);
			return 1;
		}
	...
	free(debug_output_file); debug_output_file = NULL;
	free(daily_output_file); daily_output_file = NULL;
	free(monthly_output_file); monthly_output_file = NULL;
	free(annual_output_file); annual_output_file = NULL;
	free(soil_output_file); soil_output_file = NULL;
