...
void Turnover (SPECIES *s, CELL *c, int DaysInMonth, int height)
{
	...
	if (s->counter[VEG_UNVEG] == 1)
	{
		...
		if (s->value[PHENOLOGY] == 1.1 || s->value[PHENOLOGY] == 1.2)
		{
			...
			s->value[BIOMASS_FOLIAGE_tDM] -= (s->value[BIOMASS_FOLIAGE_tDM] * s->value[LITTERFALL_RATE]);
			Log("Foliage Biomass after turnover = %f\n", s->value[BIOMASS_FOLIAGE_tDM]);
			s->value[DAILY_DEL_LITTER] += (s->value[BIOMASS_FOLIAGE_tDM] * s->value[LITTERFALL_RATE]);
...
//		Log("Dead Stem Biomass = %f tDM/cell\n", s->value[BIOMASS_STEM_DEAD_WOOD]);
		...
		s->value[BIOMASS_STEM_LIVE_WOOD_tDM] -= (s->value[DEL_STEMS] * (s->value[LIVE_WOOD_TURNOVER]/(int)s->counter[DAY_VEG_FOR_LITTERFALL_RATE]));
		Log("Daily live stem wood passing to dead stem wood = %f tDM/cell\n", s->value[BIOMASS_STEM_LIVE_WOOD_tDM] * (s->value[LIVE_WOOD_TURNOVER]/(int)s->counter[DAY_VEG_FOR_LITTERFALL_RATE]));
		Log("Live Stem Biomass = %f tDM/cell\n", s->value[BIOMASS_STEM_LIVE_WOOD_tDM]);
		s->value[BIOMASS_STEM_DEAD_WOOD_tDM] += (s->value[DEL_STEMS] * (s->value[LIVE_WOOD_TURNOVER]/(int)s->counter[DAY_VEG_FOR_LITTERFALL_RATE]));
		Log("Dead Stem Biomass = %f tDM/cell\n", s->value[BIOMASS_STEM_DEAD_WOOD_tDM]);

		if (fabs((s->value[BIOMASS_STEM_LIVE_WOOD_tDM] + s->value[BIOMASS_STEM_DEAD_WOOD_tDM]) - s->value[BIOMASS_STEM_tDM])>1e-1)
		{
...
//		Log("Dead Coarse root Biomass = %f tDM/cell\n", s->value[BIOMASS_COARSE_ROOT_DEAD_WOOD]);
		...
		s->value[BIOMASS_COARSE_ROOT_LIVE_WOOD_tDM] -= (s->value[DEL_ROOTS_COARSE] * (s->value[LIVE_WOOD_TURNOVER]/(int)s->counter[DAY_VEG_FOR_LITTERFALL_RATE]));
		...
		s->value[BIOMASS_COARSE_ROOT_DEAD_WOOD_tDM] += (s->value[DEL_ROOTS_COARSE] * (s->value[LIVE_WOOD_TURNOVER]/(int)s->counter[DAY_VEG_FOR_LITTERFALL_RATE]));
...
//		Log("Dead Stem Biomass = %f tDM/cell\n", s->value[BIOMASS_STEM_BRANCH_DEAD_WOOD]);
		...
		s->value[BIOMASS_STEM_BRANCH_LIVE_WOOD_tDM] -= (s->value[DEL_BB] * (s->value[LIVE_WOOD_TURNOVER]/(int)s->counter[DAY_VEG_FOR_LITTERFALL_RATE]));
		...
		s->value[BIOMASS_STEM_BRANCH_DEAD_WOOD_tDM] += (s->value[DEL_BB] * (s->value[LIVE_WOOD_TURNOVER]/(int)s->counter[DAY_VEG_FOR_LITTERFALL_RATE]));
