...
//
//		logger(g_debug_log, "****DAILY TURNOVER following Marconi flowChart****\n");
//
...
//				logger(g_debug_log, "Daily litter fall rate = %f\n", litterfall_rate);
...
//				logger(g_debug_log, "Foliage Biomass after turnover = %f\n", s->value[BIOMASS_FOLIAGE_tDM]);
...
//				}
//				logger(g_debug_log, "++Lai from foliage or reserve = %f\n", s->value[LAI]);
...
//					logger(g_debug_log, "****Fine root turnover****\n");
...
//		logger(g_debug_log, "Daily live stem branch wood passing to dead stem branch wood = %f tDM/cell\n", s->value[BIOMASS_STEM_BRANCH_LIVE_WOOD] * (s->value[LIVE_WOOD_TURNOVER]/365.0));
//		logger(g_debug_log, "Live Stem branch Biomass = %f tDM/cell\n", s->value[OLD_BIOMASS_STEM_BRANCH_LIVE_WOOD]);
