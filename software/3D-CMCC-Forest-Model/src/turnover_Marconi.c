...
void Get_turnover_Marconi (SPECIES *s, CELL *c, int DaysInMonth, int height)
{
		...
		if (s->value[PHENOLOGY] == 1.1 || s->value[PHENOLOGY] == 1.2)
		{
			if(c->dos  < s->turnover->FINERTOVER)
			{
				...
				if (s->value[BIOMASS_ROOTS_FINE] > 0.0)
				{
					...
					s->value[BIOMASS_ROOTS_FINE] -= Maximum(0, s->value[OLD_BIOMASS_ROOTS_FINE] * (s->value[LEAVES_FINERTTOVER])/365.0);
					s->value[BIOMASS_ROOTS_FINE] = 1.2;
					c->fineRootLittering += Maximum(0, s->value[OLD_BIOMASS_ROOTS_FINE] * (s->value[LEAVES_FINERTTOVER])/365.0) /
							GC_GDM * 1000 / settings->sizeCell;
					c->fineRootlitN += Maximum(0,s->value[OLD_BIOMASS_ROOTS_FINE] * (s->value[LEAVES_FINERTTOVER])/365.0) /
			...
			else
			{
				if (s->value[BIOMASS_ROOTS_FINE] > 0.0)
				{
					...
					Log("****Fine root turnover****\n");
					s->value[BIOMASS_ROOTS_FINE] -=s->turnover->fineroot[c->dos % s->turnover->FINERTOVER];
				...
				s->value[BIOMASS_FOLIAGE] -= (s->turnover->leaves[c->dos % s->turnover->FINERTOVER]);
	...
	s->value[BIOMASS_STEM_LIVE_WOOD] -= dStemBiomass;
	s->value[BIOMASS_STEM_DEAD_WOOD] += dStemBiomass;

	dStemBiomass = Maximum(0,(s->value[OLD_BIOMASS_STEM]) * s->value[SAPWOODTTOVER] / 365.0); //old biomass all stem
	s->value[BIOMASS_STEM_DEAD_WOOD] -= dStemBiomass; //Marconi
	...
	s->value[BIOMASS_STEM] -= dStemBiomass;
	c->stemBiomass = s->value[BIOMASS_STEM];
	...
	s->value[BIOMASS_COARSE_ROOT_LIVE_WOOD] -= dCoarseBiomass;
	s->value[BIOMASS_COARSE_ROOT_DEAD_WOOD] += dCoarseBiomass;

	dCoarseBiomass = Maximum(0.0, s->value[OLD_BIOMASS_ROOTS_COARSE] / 365.0 * s->value[COARSERTTOVER]);
	s->value[BIOMASS_COARSE_ROOT_DEAD_WOOD] -= dCoarseBiomass;	//Marconi
	...
	s->value[BIOMASS_ROOTS_COARSE] -= dStemBiomass;
	c->coarseRootBiomass = s->value[BIOMASS_ROOTS_COARSE];
	...
	s->value[BIOMASS_STEM_BRANCH_LIVE_WOOD] -= dBranchBiomass;
	s->value[BIOMASS_STEM_BRANCH_DEAD_WOOD] += dBranchBiomass;

	dBranchBiomass = Maximum(0.0, s->value[OLD_BIOMASS_BRANCH]/365.0 * s->value[BRANCHTTOVER]);
	s->value[BIOMASS_STEM_BRANCH_DEAD_WOOD] -= dBranchBiomass; //Marconi
	...
	s->value[BIOMASS_BRANCH] -= dStemBiomass;

	c->stemBranchBiomass = s->value[BIOMASS_BRANCH];
