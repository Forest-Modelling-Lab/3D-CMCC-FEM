...
void soil_respiration_reichstein ( cell_t *const c, const meteo_daily_t *const meteo_daily )
{
	...
	double E0;                    /* activation-energy-type parameter of Lloyd and Taylor [1994] (K) */
	double water_scalar;
	double temp_scalar;
	double f;
...
//FIXME MOVE TO HETEROTROPHIC_RESPIRATION.C
void heterotrophic_respiration_biome ( cell_t *const c, const meteo_daily_t *const meteo_daily )
{
	...
	/* remove from total pool carbon lost for decomposition */
	c->litrC       -= litrC_lost;
	...
	/* total litrC heterotrophic respiration gC/m2/day */
	if ( litrC_lost > 0. )
	{
		//FIXME this is NOT soil respiration BUT HETEROTROPHIC RESPIRATION!!!!!
		c->daily_soil_resp      = litrC_lost * RFL1S1;
	}
	else
	{
		c->daily_soil_resp      = 0. ;
