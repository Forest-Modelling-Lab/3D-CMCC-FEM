...
{
	static HEIGHT _height = { 0 };
	static AGE _age = { 0 };
	static SPECIES _species = { 0 };
	HEIGHT *h;
	AGE *a;
	...
	if(!mystricmp(settings->management, "on"))
	{
		...
		c->heights[c->heights_count-1] = _height;
		...
		Log("avdbh sampling = %f\n", a->species[a->species_count-1].value[AVDBH]);
		/* compute density */
