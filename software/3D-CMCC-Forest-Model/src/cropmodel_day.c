...
int crop_model_D(MATRIX *const m, const YOS *const yos, const int years, const int month, const int day, const int years_of_simulation)
{
	...
	for ( cell = 0; cell < m->cells_count; cell++)
	{
		...
		//insert get_daylenght(&m->cells[cell])
		...
		if (met[month].d[day].solar_rad != -9999)
		{
			...
			Log("\n\n\nsolar radiation taken from input file: %g", met[month].d[day].solar_rad);
		...
		//albedo			fixsergio: put it in shared
		...
		//SCS CN correction method:
			...
			{
				if (day > 4) // it would be if day > 4° of the year
				{
		...
		if (stop_cycle == 0)
		{
			if (species == 0)
			{
				...
				case 0:
					...
					if ( vernalizationDays < 10.0 && maxAirTemperature > 30.0)
					{
						vernalizationDays -= (0.5 * (int)(maxAirTemperature - 30.0));
					}
