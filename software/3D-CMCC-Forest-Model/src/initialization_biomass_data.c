...
void Initialization_biomass_data (SPECIES *s, HEIGHT *h)
{
	...
	if (s->value[BIOMASS_STEM]== 0 )
	{
		...
		s->value[BIOMASS_STEM_C] = (s->value[AV_STEM_MASS_KgDM] * s->counter[N_TREE] / 1000)/2.0;
	...
	else
	{
		...
		s->value[BIOMASS_STEM_C] = s->value[BIOMASS_STEM_C] /2.0;
	}
	...
	if (s->value[BIOMASS_BRANCH]== 0 )
	{
		...
		else
		{
			...
			s->value[BIOMASS_BRANCH_C] = s->value[BIOMASS_STEM_C] * s->value[FRACBB];
	...
	else
	{
		...
		s->value[BIOMASS_BRANCH_C] = s->value[BIOMASS_BRANCH] /2.0;
	}

	/*computing total stem biomass*/
	s->value[BIOMASS_TOT_STEM] = s->value[BIOMASS_STEM] + s->value[BIOMASS_BRANCH];
	s->value[BIOMASS_TOT_STEM_C] = s->value[BIOMASS_STEM_C] + s->value[BIOMASS_BRANCH_C];
	...
	if(s->value[BIOMASS_ROOTS_COARSE]== 0)
	{
		...
		s->value[BIOMASS_COARSE_ROOT_C] = s->value[BIOMASS_STEM_C] * s->value[COARSE_ROOT_STEM];
	...
	else
	{
		...
		s->value[BIOMASS_COARSE_ROOT_C] = s->value[BIOMASS_ROOTS_COARSE]/2.0;
	}
	...
	Log("   Total Sapwood biomass per tree = %f tC tree \n", s->value[WTOT_sap_C]/s->counter[N_TREE]);
	Log("   Total Sapwood biomass per tree = %f KgC tree \n", (s->value[WTOT_sap_C]/s->counter[N_TREE])*1000.0);
	Log("   Total Sapwood biomass per tree = %f gC tree \n", (s->value[WTOT_sap_C]/s->counter[N_TREE])*1000000.0);
	...
	if (s->value[RESERVE] == 0)
	{
		...
		s->value[RESERVE_C]= s->value[WTOT_sap_C] * s->value[SAP_WRES];
		...
		Log("-----Reserve Biomass initialization data  = %f Kg res/cell \n", s->value[RESERVE] * 1000);
		Log("-----Reserve Biomass initialization data  = %f g res/tree \n", (s->value[RESERVE] * 1000000)/(int)s->counter[N_TREE]);
	...
	if (s->value[BIOMASS_FOLIAGE] == 0)
	{
		...
		else
		{
			...
			s->value[BIOMASS_FOLIAGE_C] =  s->value[RESERVE_C] * (1.0 - s->value[STEM_LEAF_FRAC]);
			Log("----Foliage Biomass initialization data  = %f \n", s->value[BIOMASS_FOLIAGE]);
		}
	}
	else
	{
		s->value[BIOMASS_FOLIAGE_C] = s->value[BIOMASS_FOLIAGE]/2.0;
	...
	if (s->value[BIOMASS_ROOTS_FINE] == 0 && (s->value[PHENOLOGY] == 1.1 || s->value[PHENOLOGY] == 1.2))
	{
		...
		s->value[BIOMASS_FINE_ROOT_C] = s->value[BIOMASS_FOLIAGE_C] * s->value[FINE_ROOT_LEAF];
		Log("---Fine Root Biomass initialization data from Stem Biomass = %f \n", s->value[BIOMASS_ROOTS_FINE]);
	}
	else
	{
		s->value[BIOMASS_FINE_ROOT_C] = s->value[BIOMASS_ROOTS_FINE]/2.0;
	...
	s->value[BIOMASS_ROOT_TOT_C] = s->value[BIOMASS_COARSE_ROOT_C] + s->value[BIOMASS_FINE_ROOT_C];
	...
	s->value[BIOMASS_STEM_LIVE_WOOD_C]= s->value[BIOMASS_STEM_C] * (s->value[LIVE_TOTAL_WOOD_FRAC]);
	...
	s->value[BIOMASS_STEM_DEAD_WOOD_C]= s->value[BIOMASS_STEM_C] -s->value[BIOMASS_STEM_LIVE_WOOD_C];
	...
	s->value[BIOMASS_COARSE_ROOT_LIVE_WOOD_C]= s->value[BIOMASS_COARSE_ROOT_C] * (s->value[LIVE_TOTAL_WOOD_FRAC]);
	...
	s->value[BIOMASS_COARSE_ROOT_DEAD_WOOD_C]= s->value[BIOMASS_COARSE_ROOT_C] -s->value[BIOMASS_COARSE_ROOT_LIVE_WOOD];
	...
	s->value[BIOMASS_BRANCH_LIVE_WOOD_C]= s->value[BIOMASS_BRANCH_C] * (s->value[LIVE_TOTAL_WOOD_FRAC]);
	...
	s->value[BIOMASS_BRANCH_DEAD_WOOD_C]= s->value[BIOMASS_BRANCH_C] -s->value[BIOMASS_STEM_BRANCH_LIVE_WOOD];
