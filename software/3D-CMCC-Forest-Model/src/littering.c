...
void littering ( cell_t *const c, species_t *const s )
{
	...
	s->value[DEADWOOD_LITR2C] = ( s->value[C_TO_DEADWOOD] * s->value[DEAD_WOOD_USCEL_FRAC]);
	s->value[DEADWOOD_LITR3C] = ( s->value[C_TO_DEADWOOD] * s->value[DEAD_WOOD_SCEL_FRAC]) ;
	s->value[DEADWOOD_LITR4C] = ( s->value[C_TO_DEADWOOD] * s->value[DEAD_WOOD_LIGN_FRAC]) ;

	/* check */
	CHECK_CONDITION ( fabs ( s->value[DEADWOOD_LITR2C] + s->value[DEADWOOD_LITR3C] + s->value[DEADWOOD_LITR4C] ), >,  s->value[C_TO_DEADWOOD] + eps);

	/*** update cell-level coarse woody debris carbon fluxes (gC/m2/day) ****/
	c->daily_cwd_to_litr2C        += s->value[DEADWOOD_LITR2C]       * 1e6 / g_settings->sizeCell;
	c->daily_cwd_to_litr3C        += s->value[DEADWOOD_LITR3C]       * 1e6 / g_settings->sizeCell;
	c->daily_cwd_to_litr4C        += s->value[DEADWOOD_LITR4C]       * 1e6 / g_settings->sizeCell;
	c->daily_cwd_to_litrC         += (s->value[DEADWOOD_LITR2C]      +
			s->value[DEADWOOD_LITR3C]                                +
			s->value[DEADWOOD_LITR4C])                               *
					1e6 / g_settings->sizeCell;

	/*** compute cell-level coarse woody debris carbon pools (gC/m2) ***/
	c->cwd_litrC   += s->value[C_TO_DEADWOOD]           * 1e6 / g_settings->sizeCell;
	...
	s->value[DEADWOOD_LITR2N] = ( s->value[N_TO_DEADWOOD] * s->value[DEAD_WOOD_USCEL_FRAC]);
	s->value[DEADWOOD_LITR3N] = ( s->value[N_TO_DEADWOOD] * s->value[DEAD_WOOD_SCEL_FRAC]) ;
	s->value[DEADWOOD_LITR4N] = ( s->value[N_TO_DEADWOOD] * s->value[DEAD_WOOD_LIGN_FRAC]) ;

	/* check */
	CHECK_CONDITION ( fabs ( s->value[DEADWOOD_LITR2N] + s->value[DEADWOOD_LITR3N] + s->value[DEADWOOD_LITR4N] ), >,  s->value[N_TO_DEADWOOD] + eps);
	...
	c->cwd_litrN   += s->value[N_TO_DEADWOOD]           * 1e6 / g_settings->sizeCell;
	...
	c->daily_stem_cwdN            += s->value[N_STEM_TO_DEADWOOD]    * 1e6 / g_settings->sizeCell;
	c->daily_croot_cwdN           += s->value[N_CROOT_TO_DEADWOOD]   * 1e6 / g_settings->sizeCell;
	c->daily_branch_cwdN          += s->value[N_BRANCH_TO_DEADWOOD]  * 1e6 / g_settings->sizeCell;
	c->daily_reserve_cwdN         += s->value[N_RESERVE_TO_DEADWOOD] * 1e6 / g_settings->sizeCell;
	c->daily_fruit_cwdN           += s->value[N_FRUIT_TO_DEADWOOD]   * 1e6 / g_settings->sizeCell;
	c->daily_cwd_to_litrN         += (s->value[N_STEM_TO_DEADWOOD]   +
	...
	/* cell level litter carbon fluxes */
	c->daily_cwd_to_litr2N        += s->value[DEADWOOD_LITR2N]   * 1e6 / g_settings->sizeCell;
	c->daily_cwd_to_litr3N        += s->value[DEADWOOD_LITR3N]   * 1e6 / g_settings->sizeCell;
	c->daily_cwd_to_litr4N        += s->value[DEADWOOD_LITR4N]   * 1e6 / g_settings->sizeCell;
