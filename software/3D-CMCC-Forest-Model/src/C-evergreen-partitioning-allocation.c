		...
		if (npp_to_alloc > 0.0)
		{
			...
			if(s->value[RESERVE_C] >= s->value[MIN_RESERVE_C])
			{
				...
				if ( ( ( s->value[C_TO_LEAF] * ( 1. - s->value[EFF_GRPERC] ) ) + s->value[LEAF_C]) > s->value[MAX_LEAF_C])
				{
					...
					s->value[C_TO_RESERVE] += ( ( npp_to_alloc * ( 1. - s->value[FROOT_LEAF_FRAC] )) - ( max_leafC + ( max_leafC * s->value[EFF_GRPERC] ) ) );
				}
				...
				if ( ( (s->value[C_TO_FROOT] * ( 1. - s->value[EFF_GRPERC] ) ) + s->value[FROOT_C] ) > s->value[MAX_FROOT_C])
				{
					...
					s->value[C_TO_RESERVE] += ( ( npp_to_alloc * s->value[FROOT_LEAF_FRAC] ) - ( max_frootC + ( max_frootC * s->value[EFF_GRPERC] ) ) );
				}
				/********************************************************************************************************************************************/
			}
			/* it needs */
