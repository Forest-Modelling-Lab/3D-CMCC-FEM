...
void soil_nitrogen_decomp(cell_t *const c, const meteo_daily_t *const meteo_daily)
{
	int ok=1;
	...
	*/
	...
	{
		w_scalar = log(minpsi/c->psi)/log(minpsi/maxpsi);
	}
	...
	*/
	...
	if (c->litr1n > 0.0) cn_l1 = cs->litr1c/ns->litr1n;
	if (c->litr2n > 0.0) cn_l2 = cs->litr2c/ns->litr2n;
	if (c->litr4n > 0.0) cn_l4 = cs->litr4c/ns->litr4n;
	cn_s1 = SOIL1_CN;
	cn_s2 = SOIL2_CN;
	cn_s3 = SOIL3_CN;
	cn_s4 = SOIL4_CN;
